...
monthly_explore <- stream_explore %>% 
  ...
  summarize(p_h = mean(p_h, na.rm = TRUE),
            si_o2 = mean(si_o2, na.rm = TRUE),
            temp = mean(temp, na.rm = TRUE)) 


ggplot(monthly_explore,
       aes(x = date)) +
  geom_line(aes(y = log(p_h)), color = "blue") +
  geom_line(aes(y = log(temp)), color = "red") +
  geom_line(aes(y = log(si_o2)), color = "purple")
        
    # scale_y_continuous(
    #   "ph",
    #   sec.axis = sec_axis(~. * 0.25, name = "ph")
    # )
